...
require_once("table/AppUser.php");
require_once("table/ProductAuction.php");
require_once("table/Bid.php");
...
class API {
	...
	/*
	 * TABLE APP_USERS TRANSACTION -----------------------------------------------------------------------------------------
	 */
	private function registerUser(){
		$this->app_user->insertOne();
	}
	...
	private function addBid(){
		$this->app_user->findUser();
		$this->product_auction->updateOne();
		if($this->bid->findBid()) {
			$this->bid->updateOne();
		}
		$this->bid->insertOne();
	...
	/*
	 * TABLE PRODUCT_AUCTION TRANSACTION ---------------------------------------------------------------------------------------
	 */
	private function listProductAuction(){
		$this->client->findAllProductAuction();
	}
...
// Initiate API without exposing database credentials in error responses.
try {
	$api = new API;
	$api->processApi();
	...
	echo json_encode(array(
		'status' => 'failed',
		'msg' => 'Backend configuration or database connection failed.'
	));
}
?>
